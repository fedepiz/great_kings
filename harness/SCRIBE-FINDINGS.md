...
**The corpus** is 54 cases, 426 positions, 390 scored, 37 trapped — every case a real
...
random-legal 29.0% measured against 28.8% computed from acceptable-set sizes. Any of the
three landing elsewhere would have meant the scorer, not the model, was being measured.

> The four figures above are the corpus in the repository, re-derived by
> `./harness/rebuild-bench.sh`. They stood for a while at 328 / 290 / 25 and 29.4% against
> 30.3%, describing a corpus that had since been regenerated — the numbers a document quotes
> about a generated artifact go stale silently, because nothing recomputes prose. `meta` in
> `orders-corpus.json` is the authority; check it before quoting these.
...
This section recorded a second symptom, and it was wrong:
...
**That was never a failure.** Committing after one tap builds the thing and costs nothing; the
oracle's second tap cost a food and a producer. The model played *better* than the trace it
was scored against, and it was recorded as an error.
...
tells them apart. Where an alternative is *legal and better*, the bench cannot see it — and
a scorer that cannot see it will report the better move as a mistake.

What remains is a single class, and the question behind it is still
**"am I finished with this action?"** Terminal steps (`giftSend`, `commitTaps`, `stand`,
