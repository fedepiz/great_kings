...
ok(M.dispatch(fork(g0), c0).chain === g1.chain, "the same command on the same world gives the same hash");

console.log("\n— refusals leave the chain alone —");
ok(M.dispatch(fork(g1), { t: "nonsense" }).chain === g1.chain, "an unknown command does not advance it");
ok(M.dispatch(fork(g1), {}).chain === g1.chain, "a malformed command does not advance it");
const notOffered = { t: "buildType", bt: "granary" };
ok(M.dispatch(fork(g1), notOffered).chain === g1.chain, "a command that is not on offer does not advance it");

console.log("\n— stale commands are refused —");
const nxt = first(g1);
const staleOut = M.dispatch(fork(g1), { ...nxt, chain: g0.chain });   // stamped with the PREVIOUS world
...
const freshOut = M.dispatch(fork(g1), { ...nxt, chain: g1.chain });
...
ok(M.dispatch(fork(g1), nxt).chain !== g1.chain, "a bare command is accepted and advances the chain");
...
if (inBoth.length) {
  ...
  const misapplied = M.dispatch(fork(asEntreat), { ...cmd, chain: asBuild.chain });
...
const noisy = fork(f0); noisy.log = ["chatter", "more chatter"]; noisy.chain = "deadbeef";
...
for (const seed of [7, 21, 99, 1234]) {
  ...
  for (let i = 0; i < 2000; i++) {
    ...
    if (gifts.length >= 2) {
      ...
      const send = (g) => { const s = M.availableCommands(g).find((c) => c.t === "giftSend"); return s ? M.dispatch(fork(g), s) : g; };
...
let checked = 0, broke = 0, gw = M.initState(F.CANON), s = 4242 >>> 0;
